...
把imm（立即数）左移12位并带符号扩展到64位后，得到一个新的立即数。这个新的立即数是一个有符号的立即数，再加上当前PC值，然后存储到rd寄存器中。由于新的立即数表示的是地址的高20位部分，并且是一个有符号的立即数，因此这条指令的寻址范围为基于当前的PC偏移量±2 GB，如图3.6所示。另外，由于这个新的立即数的低12位都是0，因此它只能寻址到与4 KB对齐的地址。涉及4 KB以内的寻址，则需要结合其他指令（如ADDI指令）来完成。
